...
## Pausing / Resuming between SIGs

The `processRideReports` function is driven by an EventBridge schedule rule. When
the SIG is paused, that rule is disabled directly via the AWS CLI (no redeploy
needed). With the schedule off, nothing publishes to SNS, so `postRideReportToSlack`
goes idle too. The functions stay deployed and cost nothing while idle.

**Current status: PAUSED on 2026-06-22 — resume in spring.**

To resume in the spring, re-enable the production schedule rule:

`aws events enable-rule --region us-east-1 --name "dsig-airtable-service-pro-ProcessRideReportsEventsR-ruGPzf1Yqf3V"`

To pause again, swap `enable-rule` for `disable-rule` in the command above.

To check the current state of the schedule rules:

`aws events list-rules --region us-east-1 --query "Rules[?contains(Name, 'dsig-airtable')].[Name,State,ScheduleExpression]" --output table`

Note: the rule name is generated by CloudFormation and can change if the stack is
recreated. If the name above no longer matches, look it up with the `list-rules`
command. Also note `serverless.yml` still has the schedule `enabled: true`, so a
`serverless deploy -s production` will re-enable the schedule regardless of this CLI state.
...
4. Make sure the job schedule is enabled in serverless.yml (and that the EventBridge rule is enabled — see Pausing / Resuming)

## When the SIG ends
1. Disable the production EventBridge schedule rule (see Pausing / Resuming), or disable the job schedule in serverless.yml and redeploy
